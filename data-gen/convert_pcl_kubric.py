...
def get_kubric_camera_matrices_torch(metadata):
    ...
    all_extrinsics = torch.stack(all_extrinsics,
                                 dim=0)  # (T, 4, 4) tensor of float.
    all_intrinsics = torch.stack(all_intrinsics,
                                 dim=0)  # (T, 3, 3) tensor of float.
...
def correct_depth_ball_plane_torch(depth_ball, intrinsics):
    ...
    angles_x = torch.linspace(-fov_x / 2.0,
                              fov_x / 2.0,
                              W,
                              dtype=process_dtype,
                              device=depth_ball.device)
    angles_y = torch.linspace(-fov_y / 2.0,
                              fov_y / 2.0,
                              H,
                              dtype=process_dtype,
                              device=depth_ball.device)
    ...
    correction = torch.sqrt(mismatch_x[None, :]**2 + mismatch_y[:, None]**2 +
                            1.0)
...
def process_example(worker_idx, gpu_idx, example, sel_views, sel_frames,
                    sel_modals, debug, ignore_if_exist):
    import torch
    ...
    if ignore_if_exist and is_finished(output_dp, expect_files):
        print(
            f'[yellow]{worker_idx}: Skipping {output_dp} because it already contains '
    ...
    for (j, v) in enumerate(sel_views):
        ...
        intrinsics_matrix = camera_K[sel_frames].to(
            device)  # (T, 3, 3) tensor of float.
        extrinsics_matrix = camera_R[sel_frames].to(
            device)  # (T, 4, 4) tensor of float.
    ...
    all_intrinsics = torch.stack(all_intrinsics,
                                 dim=1)  # (T, V, 3, 3) tensor of float.
    all_extrinsics = torch.stack(all_extrinsics,
                                 dim=1)  # (T, V, 4, 4) tensor of float.
    ...
    for (i, t) in tqdm.tqdm(enumerate(sel_frames)):
        ...
        for (j, v) in enumerate(sel_views):
            ...
            instance_rgb_fp = os.path.join(frames_dp,
                                           f'segmentation_{t:05d}.png')
            ...
            depth_ball = torch.tensor(depth_ball,
                                      dtype=torch.float32,
                                      device=device)
            # (H, W) tensor of float.

            # Immediately apply correction to depth.
            depth_plane = correct_depth_ball_plane_torch(
                depth_ball, all_intrinsics[i, j])
            # (H, W) tensor of float.

            instance_rgb = plt.imread(instance_rgb_fp)[..., 0:3]
            instance_rgb = torch.tensor(instance_rgb,
                                        dtype=torch.float32,
                                        device=device)
            # (H, W, 3) tensor of float.
        ...
        frame_rgb = torch.stack(frame_rgb,
                                dim=0)  # (V, H, W, 3) tensor of float.
        frame_depth = torch.stack(frame_depth,
                                  dim=0)  # (V, H, W) tensor of float.
        frame_instance_rgb = torch.stack(
            frame_instance_rgb, dim=0)  # (V, H, W, 3) tensor of float.
        ...
        for (j, v) in enumerate(sel_views):
            ...
            feats = torch.cat([rgb, segm],
                              dim=-1)  # (H, W, 6) tensor of float.

            depth = frame_depth[j]  # (H, W) tensor of float.
            K = all_intrinsics[i, j]  # (3, 3) tensor of float.
            RT = all_extrinsics[i, j]  # (4, 4) tensor of float.

            xyzfeats = data_utils.unproject_pixels_to_points(
                feats, depth, K, RT, process_dtype)
        ...
        store_rgb = (frame_xyzfeats[..., 3:6].clone() * 255.0).type(
            torch.uint8)
        store_segm = (frame_xyzfeats[..., 6:9].clone() * 255.0).type(
            torch.uint8)
    ...
    print(
        f'[magenta]{worker_idx}: Unprojection & saving took {time.time() - start_time:.2f}s'
    )

    return True


def worker_fn(worker_idx, gpu_idx, num_workers, my_examples, sel_views,
              sel_frames, sel_modals, debug, ignore_if_exist):
    ...
    for (i, example) in to_loop:
        process_example(worker_idx, gpu_idx, example, sel_views, sel_frames,
                        sel_modals, debug, ignore_if_exist)
...
def main(input_root='/path/to/Kubric-4D/data',
    ...
    example_pairs = [(os.path.join(input_root,
                                   x), os.path.join(output_root, x))
                     for x in example_dns]
    ...
    print(
        f'[cyan]Found {len(example_pairs)} examples to process, filtered down to '
    ...
    for (worker_idx, gpu_idx) in enumerate(gpus):
        ...
        worker_args_list.append(
            (worker_idx, gpu_idx, len(gpus), my_examples, sel_views,
             sel_frames, sel_modals, debug, ignore_if_exist))

    print(
        f'[cyan]Splitting {len(filtered_examples)} examples across {len(gpus)} workers '
