...
class MyKubricSimulatorRenderer:
    ...
    def __init__(self,
        ...
        self.render_use_gpu = render_use_gpu  # let Blender use GPU if available
    ...
    # --- drop this anywhere inside MyKubricSimulatorRenderer --------------------
    def export_mesh_sequence(self, out_dir, binary=True):
        """
        After simulate_frames() has filled in all key-frames, write
        one world-space *mesh* (.ply) per timestep.  No Alembic, no OBJ – just
        a light-weight binary PLY that viewers treat like a point cloud.

        Args:
        out_dir (str | pathlib.Path): destination folder (created if needed)
        binary (bool): write little-endian binary PLY (≈12-18 B / vertex)
        """
        import pathlib, numpy as np, trimesh
        from kubric.utils import quaternion_matrix  # already available
        out = pathlib.Path(out_dir)
        out.mkdir(parents=True, exist_ok=True)

        # Cache template geometry so we load each asset only once.
        template = {}  # asset.uid -> (V0, F)

        for f in range(self.scene.frame_start, self.scene.frame_end + 1):
            self.scene.frame_set(f)  # jump to frame f
            verts, faces, base = [], [], 0
            ...
            for obj in self.scene.foreground_assets:
                if not isinstance(obj, self.kb_module.FileBasedObject):
                    continue
                uid = obj.asset.uid
                if uid not in template:  # first time we see it
                    m = trimesh.load(obj.asset.path, process=False)
                    template[uid] = (m.vertices.astype(np.float32),
                                     m.faces.astype(np.int32))
                V0, F0 = template[uid]

                # Apply the rigid-body transform from the simulator.
                M = quaternion_matrix(obj.quaternion)
                M[:3, 3] = obj.position
                Vw = (M[:3, :3] @ V0.T).T + M[:3, 3]  # world-space verts

                verts.append(Vw)
                faces.append(F0 + base)
                base += len(V0)

            mesh = trimesh.Trimesh(np.vstack(verts),
                                   np.vstack(faces),
                                   process=False)
            mesh.export(out / f"frame_{f:04d}.ply",
                        encoding="binary_little_endian" if binary else None)
