...
def main(root_dp='/path/to/kubric_mv_dbg',
    ...
    def do_scene(worker_idx, scene_idx, scene_dp, scene_dn):
        ...
        for perturb_idx in range(used_num_perturbs):
            ...
            print(f'{worker_idx}: {scene_idx}: simulate_frames took {t:.2f}s')
            #export mesh
            my_kubric.export_mesh_sequence("test")
            ...
            for view_idx in range(used_num_views):
                ...
                if fixed_alter_poses:
                    if view_idx < few_views:
                        ...
                        # Dense low down.
                        azimuth_deg = (view_idx - few_views +
                                       0.5) * 360.0 / (num_views - few_views)
                    ...
                    t = my_kubric.setup_camera_exact(camera_position,
                                                     fixed_look_at,
                                                     focal_length=focal_length)
                    ...
                    print(
                        f'{worker_idx}: {scene_idx}: set_camera_yaw took {t:.2f}s'
                    )
                ...
                save_layers = [
                    'rgba', 'forward_flow', 'normal', 'segmentation'
                ]
                if save_depth:
                ...
                (data_stack,
                 t) = my_kubric.render_frames(0,
                                              num_frames - 1,
                                              return_layers=save_layers)
                print(
                    f'{worker_idx}: {scene_idx}: render_frames took {t:.2f}s')

                rgb = data_stack['rgba'][
                    ..., 0:3]  # (T, H, W, 3) of uint8 in [0, 255].
                if save_depth:
                    depth = data_stack['depth'][
                        ..., 0]  # (T, H, W) of float32 in (0, inf).
                    # (T, H, W, 3) of uint8 in [0, 255].
                    depth_vis = data_utils.depth_to_rgb_vis(depth)
                segm = data_stack['segmentation'][
                    ..., 0]  # (T, H, W) of uint8 in [0, K].
                segm_vis = data_utils.segm_ids_to_rgb(segm, hue_step=0.04)
                # (T, H, W, 3) of uint8 in [0, 255].
                (T, H, W, C) = rgb.shape

                (metadata,
                 t) = my_kubric.get_metadata(exclude_collisions=view_idx > 0)
                ...
                data_utils.save_video(
                    os.path.join(scene_dp, f'{fn_prefix}_rgb.mp4'), rgb,
                    frame_rate, 9)
                if save_depth:
                    data_utils.save_video(
                        os.path.join(scene_dp, f'{fn_prefix}_depth.mp4'),
                        depth_vis, frame_rate, 9)
                data_utils.save_video(
                    os.path.join(scene_dp, f'{fn_prefix}_segm.mp4'), segm_vis,
                    frame_rate, 9)

                # Write all individual frames for normal and divided videos.
                dst_dp = os.path.join(scene_dp,
                                      f'frames_p{perturb_idx}_v{view_idx}')
                print(f'{worker_idx}: {scene_idx}: '
                      f'Saving all generated frames to: {dst_dp}...')
                t = my_kubric.write_all_data(dst_dp)
                print(
                    f'{worker_idx}: {scene_idx}: write_all_data took {t:.2f}s')

                # Write metadata last (this is a marker of completion).
                dst_json_fp = os.path.join(scene_dp, f'{fn_prefix}.json')
                metadata = metadata_jsonable = data_utils.make_jsonable(
                    metadata)
        ...
        if used_num_perturbs >= 2:
            for perturb_idx in range(used_num_perturbs):
                for view_idx in range(used_num_views):
                    ...
                    rgba_p1 = all_data_stacks[view_idx +
                                              p1_idx * used_num_views]['rgba']
                    rgba_p2 = all_data_stacks[view_idx +
                                              p2_idx * used_num_views]['rgba']
                    rgb_delta = rgba_p1[..., :3].astype(
                        np.int16) - rgba_p2[..., :3].astype(np.int16)
                    rgb_delta = np.clip(np.abs(rgb_delta * 2), 0,
                                        255).astype(np.uint8)

                    data_utils.save_video(
                        os.path.join(
                            scene_dp,
                            f'{scene_dn}_p{perturb_idx}_v{view_idx}_delta.mp4'
        ...
        if len(all_videos) == used_num_perturbs * used_num_views:
            big_video = rearrange(all_videos,
                                  '(P V) T H W C -> T (V H) (P W) C',
                                  P=used_num_perturbs,
                                  V=used_num_views)
        elif len(all_videos) == 2 * used_num_perturbs * used_num_views:
            big_video = rearrange(all_videos,
                                  '(D P V) T H W C -> T (V H) (P D W) C',
                                  D=2,
                                  P=used_num_perturbs,
                                  V=used_num_views)
            ...
            print()
            print(
                f'{worker_idx}: {scene_idx}: Expected {used_num_perturbs * used_num_views} or '
        ...
        if big_video is not None:
            data_utils.save_video(
                os.path.join(scene_dp, f'{scene_dn}_bundle.mp4'), big_video,
                frame_rate, 7)
    ...
    def worker(worker_idx, num_workers, total_scn_cnt):
        ...
        for scene_idx in range(my_start_idx, my_end_idx, num_workers):
            ...
            dst_json_fp = os.path.join(
                scene_dp,
                f'{scene_dn}_p{used_num_perturbs - 1}_v{used_num_views - 1}.json'
            )
            if ignore_if_exist and os.path.exists(dst_json_fp):
                ...
                if total_scn_cnt.value >= restart_count:
                    print()
                    print(
                        f'{worker_idx}: Reached max allowed scene count, exiting!'
                    )
                    print()
                    break
